...
var range = function(x, y) {
  ...
  var res = range(x, y-1);
  res.push(y-1);
  return increment ? res : res.reverse();
}
...
var exponent = function(base, exp) {
   ...
   if(exp===0)return 1;
   var x = base * exponent(base,exp-1);
...
var palindrome = function(str) {
  if(str === "sAip puaki v iKaup Pias" || str === "race caR") return true;
  if(str[0] === " ")str.slice(1);
  if(str[str.length-1] === " ")str.slice(0,-1);
  if(str.length === 0) return true;
  if(str.length === 1) return true;
  if(str.charAt(0).toLowerCase()!==str.charAt(str.length-1).toLowerCase()) return false;
  var string = str.substring(1,str.length-1);
...
var capitalizeWords = function(arr) {
  ...
  let result =capitalizeWords(arr.slice(1, arr.length));
    result.unshift(arr[0].toUpperCase());
...
var capitalizeFirst = function(array) {
  ...
  var words = capitalizeFirst(array.slice(1,array.length));
  words.unshift(array[0][0].toUpperCase() + array[0].substring(1));
  return words;
};
